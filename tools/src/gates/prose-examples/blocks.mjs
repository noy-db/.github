/**
 * Block extraction + the preamble convention for the `prose-examples` gate.
 *
 * ⭐ PORTED VERBATIM from noy-db/core's `scripts/prose-examples/blocks.mjs`,
 * which the family ruled the reference implementation (family#26). It was
 * already pure — no filesystem, no tsc, no repo-specific path — so nothing
 * here needed adapting, and nothing here should acquire a repo-specific
 * branch: the two decisions it owns (which fenced blocks exist, and what code
 * each compiles as) are the same in every repo.
 *
